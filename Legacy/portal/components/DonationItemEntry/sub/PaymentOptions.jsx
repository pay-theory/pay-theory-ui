...
import { TextEntryCurrency, FormHead } from '../../'
		...
		<BooksHooks.context.paymentItem.Consumer>
			{paymentHook => {
				const changeOptions = (event, index, changePayment) => {
					...
					paymentItem.item_quick_pay_options[index] =
						event.target.value
				...
				const amountOptionSelected = event => {
					const paymentItem = { ...paymentHook }
					paymentItem.item_has_fixed_amount =
				...
				const options_array = paymentHook.item_quick_pay_options
					? paymentHook.item_quick_pay_options
					: []

				/* istanbul ignore next */
				const options =
					options_array.length === 3
						? options_array
						: [1.0, 5.0, 10.0]
