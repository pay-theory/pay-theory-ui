...
const parse = (uploaded, forDistrict) => {
	...
	while (rows.length - 1) {
		...
		if (row.LastName && row.AccountCode && school) {
			...
			const receipts =
				usas_submissions[school.toString()][account_code.toString()].receipts
			receipts.push({
				school: school,
			...
			const fees =
				parseFloat(
					usas_submissions[school.toString()][account_code.toString()].fees
				) + fee
			...
			school_fees[school.toString()] = school_fees[school.toString()] + fee
