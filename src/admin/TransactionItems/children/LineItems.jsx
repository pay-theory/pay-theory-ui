...
import { CardRow, Checkbox } from '../../../common'
					...
					<BooksHooks.context.paymentItems.Consumer>
						{(itemsHook) => {
							paymentHook.items.forEach((pItem) => {
								...
								prepareditems[pItem.invoice_item_uid].amount = pItem.amount
								subtotal = subtotal + parseFloat(pItem.amount)
							})
							total = subtotal + subtotal * taxPct
							itemsHook.forEach((fItem) => {
								prepareditems[fItem.key].title = fItem.data.item_title
								prepareditems[fItem.key].type = fItem.data.item_type
							})
										...
										<div className='table-row-head'>
											<span className='head purchase-item-selector'>
												<Checkbox
													id='select-all-items'
													indeterminate={
														selected.length < paymentHook.items.length &&
														selected.length > 0
													}
													checked={
														selected.length === paymentHook.items.length &&
													...
													onChange={(e) => {
														if (e.target.checked) {
															setSelected(paymentHook.items.slice(0))
														} else {
															setSelected([])
														}
													}}
											...
											<span className='head purchase-item'>Item</span>
											<span className='head product-type'>Type</span>
											<span className='head quantity'>Qty</span>
											<span className='head right total'>Total</span>
										</div>
										<div className='table inner'>
											{paymentHook.items.map((item) => {
												return (
													<div
														className='table-row'
														key={item.invoice_item_uid}
													>
														<span className='cell purchase-item-selector'>
															<Checkbox
																data-testid='select-item'
																onChange={(e) => {
																	setSelected(establishSelection(e, item))
																}}
																checked={selected.includes(item)}
															/>
														</span>
														<span className='cell purchase-item'>
															{prepareditems[item.invoice_item_uid].title}
														</span>
														<span className='cell product-type'>
															{prepareditems[item.invoice_item_uid].type}
														</span>
														<span className='cell quantity'>1</span>
														<span className='cell right total'>{`$${parseFloat(
															prepareditems[item.invoice_item_uid].amount
														).toFixed(2)}`}</span>
													</div>
												)
											})}
											...
											<div className='items-footer-col'>
												<div className='refund-table-totals'>
													<p className='font-heavy navy'>
														({selected.length}) Items selected totaling:{' '}
													...
													<button
														...
														onClick={(e) => props.onRefund(selected)}
													>
